  ...
  const rejectRequest= async()=>{
    ...
    if(res.ok){
      window.location.reload()
        ...
        return (
            <>
              <section className='bg-white h-32 md:h-16 my-3 w-[95vw] md:w-[80vw] lg:w-[50vw] rounded-lg shadow flex md:flex-row flex-col md:items-center md:justify-between'>
                <span className='flex ml-4 md:py-0 py-3'>
                    <img className=' w-14 h-14 md:w-11 md:h-11 rounded-lg md:rounded-full' src={`https://res.cloudinary.com/dcgjy3xv7/image/upload/v1687762741/${friendImage}`} alt='Notification' height={100} width={100}/>

...
export default function UserCard({profileImage,id,firstName,lastName,emailOfUser}:UserCard) {
  ...
  const sendNotification= async() => {
    const res = await fetch(`${backendURL}/user/sendNotifiaction/${id}`,{
      method:'PUT',
      headers:{
        ...
        <>
          ...
          <div className=" w-56 md:w-[20vw] lg:w-[16vw] bg-white border border-gray-300 rounded-lg shadow ">
            <div className="flex flex-col items-center lg:pb-6 pb-3">
                <Image className=" w-32 h-32 md:w-16 md:h-16 lg:w-24 lg:h-24 mb-3 rounded-full shadow-lg mt-3 lg:mt-6" src={`https://res.cloudinary.com/dcgjy3xv7/image/upload/v1687762741/${profileImage}`} alt="Friend" width={100} height={100}/>
